---
section: devlog
draft: false
title: Lyn (First Protagonist)
permalink: lyn
date: 2026-09-22
tags: []
featured: true
cover: /media/lyn-front-preview-6a955fa64a01e2b5356bc0b62a5212ca.webp
coverAlt: Lyn
media:
  - type: image
    src: /media/image-preview-7b8b096f8ca8ce125291cf3c85032e8b.webp
    alt: Close up
    caption: ""
  - type: image
    src: /media/lyn-side-preview-ed41a54fb52a017b0d84bd960729bb6c.webp
    alt: Perspective
    caption: ""
  - type: image
    src: /media/lyn-rear-preview-088f176c2bd1ebbc2e856012cfcb0c49.webp
    alt: Perspective
    caption: ""
  - type: image
    src: /media/weapon1-preview-b9412a994186d81d3419793f76d7cba5.webp
    alt: Memory Blade
    caption: ""
---
In Proto: ReCompute, there will be 2 protagonists that you'll be able to play as and switch on the fly.

The first one I want to introduce is Lyn, a 19 year old CS student starting her summer vacation. Due to some unfortunate turn of events, she spends her vacation inside a computer world. Her name was chosen due to my tendency to overthink things, actually. I wanted to have a simple name after hours of coming up with one. I wanted something with an 'L'... and she is left handed. Thus the name Lyn was born.

Each protagonist will be able to equip three weapons which also have their own respective colours. Without spoiling the others, the first weapon the player gets introduced to is the Memory Blade with the lightning element. A blade was chosen intentionally to ease the beginning of the game for the player as this weapon plays in a classical DMC sense; light and heavy attacks including abilities. Other weapons will have their own movesets and mechanics however.

Let's talk about her striking design! The weapon carries the element lightning which is green in this game, thus her outfit conveys this element. Inspiration was taken from 2B from NieR: Automata which I really liked back when I played it the first time; an open airy outfit that is also revealing to some degree, including some tattoos which will play a big role in the HUD later. This was intentional to make her look badass and confident with the blade and how she zippzzz through enemies! The weapon design was inspired by Xenoblade 1's Monado, mixed with the iconic design of the Master Sword from Zelda, though it has a very computerly design as well to tell the player again that they are in a computer world.

Credit goes to the artist [Bre - Open For Big Project | Profile | Fiverr](https://www.fiverr.com/peanutsbee_lab?source=inbox)  
Thank you for your help!
