...
2. Save its public beacon token in `src/content/analytics.json`. This is a public collection identifier, not an API credential. The public-site layout loads the beacon once, with Cloudflare's automatic SPA support; the owner's writing page `/write/` has no beacon, and a browser signed in to the editor never loads it (no outside script runs where the editor sign-in lives). The site-wide Content-Security-Policy (scripts/apply-csp.mjs) allows the beacon's two Cloudflare hosts.
